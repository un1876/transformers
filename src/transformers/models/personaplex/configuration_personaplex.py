...
@auto_docstring(checkpoint="kmhf/hf-personaplexko")
@strict
class PersonaplexDepthConfig(PreTrainedConfig):
    ...
    ...     PersonaplexDepthDecoder,
    ... )

    >>> configuration = PersonaplexDepthConfig()

    >>> # Initializing a PersonaplexDepthDecoder (with random weights) from the kmhf/hf-personaplexko style configuration
    >>> model = PersonaplexDepthDecoder(configuration)
...
class PersonaplexConfig(PreTrainedConfig):
    ...
    >>> configuration = PersonaplexConfig()

    >>> # Initializing a PersonaplexForConditionalGeneration (with random weights) from the kmhf/hf-personaplexko style configuration
    >>> model = PersonaplexForConditionalGeneration(configuration)

    >>> # Accessing the model configuration
    >>> configuration = model.config

    >>> # Saving the model, including its configuration
    >>> model.save_pretrained("kmhf/hf-personaplexko")

    >>> # loading model and config from pretrained folder
    >>> personaplex_config = PersonaplexConfig.from_pretrained("kmhf/hf-personaplexko")
    >>> model = PersonaplexForConditionalGeneration.from_pretrained("kmhf/hf-personaplexko", config=personaplex_config)
    ```"""
