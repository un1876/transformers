...
from .configuration_personaplex import PersonaplexConfig, PersonaplexDepthConfig
...
class PersonaplexFlexibleLinear(nn.Module):
    ...
    def forward(self, x, layer_idx=None):
        """
        `PersonaplexFlexibleLinear` creates one linear layer per codebook. There's multiple ways to use it.
...
class PersonaplexLinear(nn.Module):
    def __init__(self, input_dim, output_dim, num_codebooks, use_flexible_linear=False):
        ...
        if not use_flexible_linear:
            ...
            self.linear = PersonaplexFlexibleLinear(input_dim, output_dim, num_layers=num_codebooks)
...
class PersonaplexRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: PersonaplexConfig, device=None):
        super().__init__()
    ...
    def compute_default_rope_parameters(
        config: PersonaplexConfig | None = None,
...
class PersonaplexGatingMLP(nn.Module):
    def __init__(self, config, use_flexible_linear=False):
        ...
        if num_layers == 1:
            ...
            self.fc1 = PersonaplexFlexibleLinear(hidden_size, ffn_dim, num_layers)
            self.fc2 = PersonaplexFlexibleLinear(ffn_dim // 2, hidden_size, num_layers)
...
class PersonaplexAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(
        self, config: PersonaplexConfig, layer_idx: int | None = None, use_flexible_linear=False, use_rope=True
    ):
        super().__init__()
        self.config = config
        ...
        if use_rope:
            self.rotary_emb = PersonaplexRotaryEmbedding(config)
...
# NO LONGER EXIST Copied from transformers.models.gemma.modeling_gemma.GemmaFlashAttention2 with Gemma->Personaplex
# TODO cyril: modular
class PersonaplexDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: PersonaplexConfig, layer_idx: int, use_flexible_linear: bool, use_rope=True):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.use_flexible_linear = use_flexible_linear

        self.self_attn = PersonaplexAttention(
            config=config, layer_idx=layer_idx, use_flexible_linear=use_flexible_linear, use_rope=use_rope
        )

        self.mlp = PersonaplexGatingMLP(config, use_flexible_linear)
        self.input_layernorm = PersonaplexRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = PersonaplexRMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring
class PersonaplexPreTrainedModel(PreTrainedModel):
    config: PersonaplexConfig
    base_model_prefix = "model"
    input_modalities = ("audio", "text")
    supports_gradient_checkpointing = True
    _no_split_modules = ["PersonaplexDecoderLayer", "MimiTransformerLayer"]
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, PersonaplexFlexibleLinear):
            init.normal_(module.weight)


@auto_docstring
class PersonaplexModel(PersonaplexPreTrainedModel):
    def __init__(self, config: PersonaplexConfig):
        ...
        self.layers = nn.ModuleList(
            [
                PersonaplexDecoderLayer(config, layer_idx, use_flexible_linear=False)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.norm = PersonaplexRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
@auto_docstring(
    custom_intro="""
    `PersonaplexForCausalLM` outputs.
    """
)
@dataclass
class PersonaplexCausalLMOutputWithPast(ModelOutput):
    r"""
...
@auto_docstring(
    custom_intro="""
    The Personaplex decoder model with a text language modelling head on top. Only usable for text.
    """
)
class PersonaplexForCausalLM(PersonaplexPreTrainedModel, GenerationMixin):
    input_modalities = ("text",)

    def __init__(self, config):
        super().__init__(config)
        self.model = PersonaplexModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs,
    ) -> tuple | PersonaplexCausalLMOutputWithPast:
        r"""
        ...
        >>> model = PersonaplexForCausalLM.from_pretrained("kmhf/hf-personaplexko")
        >>> tokenizer = AutoTokenizer.from_pretrained("kmhf/hf-personaplexko")
        ...
        return PersonaplexCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
@auto_docstring(
    custom_intro="""
    Outputs of [`PersonaplexForConditionalConditionalGeneration.generate`].
    """
)
@dataclass
class PersonaplexConditionalGenerationGenerateOutput(ModelOutput):
    r"""
...
@auto_docstring(
    custom_intro="""
    `PersonaplexForConditionalGeneration` outputs.
    """
)
@dataclass
class PersonaplexConditionalGenerationOutputWithPast(ModelOutput):
    r"""
...
class PersonaplexDepthDecoder(PersonaplexPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: PersonaplexDepthConfig):
        super().__init__(config)
        ...
        self.input_projections = PersonaplexFlexibleLinear(config.input_size, config.hidden_size, config.num_codebooks)

        self.layers = nn.ModuleList(
            [
                PersonaplexDecoderLayer(config, layer_idx, use_flexible_linear=True, use_rope=False)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )

        self.lm_heads = PersonaplexFlexibleLinear(config.hidden_size, config.audio_vocab_size, config.num_codebooks)
        self._attn_implementation = config._attn_implementation
        self.gradient_checkpointing = False
        self.config = config
...
@auto_docstring(
    custom_intro="""
    The original Personaplex model with an audio encoder, a Personaplex depth decoder and a Personaplex decoder, for speech-to-speech.
    """
)
class PersonaplexForConditionalGeneration(PersonaplexPreTrainedModel, GenerationMixin):
    config: PersonaplexConfig
    ...
    def __init__(self, config: PersonaplexConfig):
        ...
        self.decoder = PersonaplexForCausalLM(config)

        self.depth_decoder = PersonaplexDepthDecoder._from_config(config.depth_decoder_config)
    ...
    def forward(
        ...
        user_audio_codes: torch.Tensor | None = None,
        personaplex_input_values: torch.FloatTensor | None = None,
        personaplex_audio_codes: torch.Tensor | None = None,
        past_key_values: Cache | None = None,
        inputs_embeds: torch.FloatTensor | None = None,
        ...
        **kwargs,
        ...
        user_audio_codes (`torch.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio user prompt for the generation. Has priority over `user_input_values` and represents the audio "tokens" of `user_input_values` once passed through the audio encoder.
        personaplex_input_values (`torch.Tensor `of shape `(batch_size, 1, audio_sequence_length), *optional*):
            The audio waveforms used as audio Personaplex prompt for the generation.
        personaplex_audio_codes (`torch.Tensor `of shape `(batch_size, num_codebooks, sequence_length), *optional*):
            The audio codes used as audio Personaplex prompt for the generation. Has priority over `personaplex_input_values` and represents the audio "tokens" of `personaplex_input_values` once passed through the audio encoder.
        inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        ...
        Examples:
        ```python
        >>> from transformers import PersonaplexForConditionalGeneration
        >>> import torch

        >>> model = PersonaplexForConditionalGeneration.from_pretrained("kmhf/hf-personaplexko")
        >>> inputs = personaplex.get_unconditional_inputs()
        ...
        if inputs_embeds is None:
            ...
            if personaplex_input_values is not None and personaplex_audio_codes is None:
                personaplex_audio_codes = self.audio_encoder.encode(
                    personaplex_input_values, num_quantizers=self.num_codebooks, **kwargs_audio_encoder
                )[0]

            audio_codes = torch.cat([personaplex_audio_codes, user_audio_codes], dim=1)
        ...
        return PersonaplexConditionalGenerationOutputWithPast(
    ...
    def _prepare_inputs_embeds_for_generation(
        ...
        personaplex_delay_pattern_mask = None
        ...
        if (
            ...
            and personaplex_input_values is None
            and personaplex_audio_codes is None
        ):
            raise ValueError(
                "You must provide at least one of `input_ids`, `user_input_values`, `personaplex_input_values`, `user_audio_codes`, `personaplex_audio_codes` or `inputs_embeds`."
            )
        ...
        if inputs_embeds is None or apply_delay_pattern_mask:
            ...
            if personaplex_input_values is not None and personaplex_audio_codes is None:
                personaplex_audio_codes = self.audio_encoder.encode(
                    personaplex_input_values, num_quantizers=self.num_codebooks
                )[0]
        ...
        if inputs_embeds is None and concat_unconditional_inputs:
            ...
            personaplex_audio_codes = torch.cat(
                [unconditional_inputs.personaplex_audio_codes, personaplex_audio_codes], dim=2
            )
        ...
        if inputs_embeds is None or apply_delay_pattern_mask:
            ...
            if apply_delay_pattern_mask and personaplex_audio_codes is not None:
                personaplex_audio_codes, personaplex_delay_pattern_mask = self.build_delay_pattern_mask(
                    personaplex_audio_codes,
                    bos_token_id=self.config.audio_vocab_size,
                    pad_token_id=self.config.audio_vocab_size,
                    max_length=generation_config.max_length,
                )
        ...
        if inputs_embeds is None:
            ...
            if user_audio_codes is not None and personaplex_audio_codes is not None:
                audio_codes = torch.cat([personaplex_audio_codes, user_audio_codes], dim=1)
                audio_inputs_embeds = sum(
                    self.embed_tokens[codebook](audio_codes[:, codebook]) for codebook in range(audio_codes.shape[1])
                )
            elif personaplex_audio_codes is not None:
                audio_codes = personaplex_audio_codes
                audio_inputs_embeds = sum(
                    self.embed_tokens[codebook](audio_codes[:, codebook]) for codebook in range(audio_codes.shape[1])
                )
        ...
        return (
            ...
            personaplex_delay_pattern_mask,
            attention_mask,
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        Return:
            [`PersonaplexConditionalGenerationGenerateOutput`]
        """
        ...
        input_ids, user_audio_codes, personaplex_audio_codes, concat_unconditional_inputs = (
            self._check_and_maybe_initialize_inputs(
                input_ids=input_ids,
                user_input_values=user_input_values,
                user_audio_codes=user_audio_codes,
                personaplex_input_values=personaplex_input_values,
        ...
        # create blank user inputs - personaplex needs a constant stream of user inputs
        ...
        kwargs["personaplex_delay_pattern_mask"] = (
            personaplex_delay_pattern_mask
            if personaplex_delay_pattern_mask is not None
            else kwargs.get("personaplex_delay_pattern_mask")
        )

        self.generated_audio_codes = torch.repeat_interleave(
            personaplex_audio_codes, max(generation_config.num_beams, generation_config.num_return_sequences), dim=0
        )
        ...
        if generation_config.num_return_sequences > 1:
            personaplex_delay_pattern_mask = torch.repeat_interleave(
                personaplex_delay_pattern_mask, generation_config.num_return_sequences, dim=0
            )
        ...
        if generation_config.num_beams > 1:
            ...
            beam_indices = outputs.beam_indices[:, : -personaplex_audio_codes.shape[-1]]

            generated_audio_codes = self.generated_audio_codes[:, :, personaplex_audio_codes.shape[-1] :]
            ...
            personaplex_audio_codes = torch.repeat_interleave(
                personaplex_audio_codes, generation_config.num_return_sequences, dim=0
            )
            self.generated_audio_codes = torch.cat((personaplex_audio_codes, generated_audio_codes), dim=2)
        ...
        output_audio_codes = self.apply_delay_pattern_mask(self.generated_audio_codes, personaplex_delay_pattern_mask)

        # revert the pattern delay mask by filtering the pad token id and bos token ids
        mask = personaplex_delay_pattern_mask != self.config.audio_vocab_size
        ...
        if generation_config.return_dict_in_generate:
            return PersonaplexConditionalGenerationGenerateOutput(
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Overwritten -- Personaplex has custom post-processing on the prepared inputs.
        ...
        model_inputs = super().prepare_inputs_for_generation(
            ...
            personaplex_delay_pattern_mask=personaplex_delay_pattern_mask,
            kwargs_depth_decoder=kwargs_depth_decoder,
        ...
        if model_inputs["input_ids"] is not None:
            ...
            self.generated_audio_codes = self.apply_delay_pattern_mask(
                torch.cat([self.generated_audio_codes, generated_audio_codes], dim=2), personaplex_delay_pattern_mask
            )

            inputs_embeds, _, _, _, _, _, _ = self._prepare_inputs_embeds_for_generation(
                input_ids,
                personaplex_audio_codes=self.generated_audio_codes[:, :, -1:],
                user_audio_codes=user_audio_codes,
            )
    ...
    def get_unconditional_inputs(self, num_samples=1):
        ...
        >>> model = PersonaplexForConditionalGeneration.from_pretrained("kmhf/hf-personaplexko-pytorch-bf16")
    ...
    def _check_and_maybe_initialize_inputs(
        ...
        personaplex_input_values=None,
        personaplex_audio_codes=None,
        inputs_embeds=None,
        ...
        personaplex_input = personaplex_audio_codes if personaplex_input_values is None else personaplex_input_values

        one_input_has_been_passed = (user_input is not None) or (personaplex_input is not None) or (inputs is not None)
        ...
        if one_input_has_been_passed and (user_input is None):
            raise ValueError(
                "No user audio inputs have been passed alongside the other inputs. Make sure either `user_input_values` or `user_audio_codes` is passed or use `PersonaplexForConditionalGeneration.get_unconditional_inputs`. Check the `PersonaplexForConditionalGeneration` docstrings for more information."
            )
        elif one_input_has_been_passed and (personaplex_input is None):
            raise ValueError(
                "No Personaplex audio inputs have been passed alongside the other inputs. Make sure either `personaplex_input_values` or `personaplex_audio_codes` is passed or use `PersonaplexForConditionalGeneration.get_unconditional_inputs`. Check the `PersonaplexForConditionalGeneration` docstrings for more information."
            )
        elif one_input_has_been_passed and (inputs is None):
            raise ValueError(
                "No `input_ids` or `inputs_embeds` have been passed alongside the other inputs. Make sure `input_ids` is passed or use `PersonaplexForConditionalGeneration.get_unconditional_inputs`. Check the `PersonaplexForConditionalGeneration` docstrings for more information."
            )
            ...
            personaplex_audio_codes = unconditional_inputs.personaplex_audio_codes
            ...
            personaplex_seq_length = personaplex_input.shape[-1]
            tokens_seq_length = inputs.shape[1]

            ratio = self.config.audio_encoder_config.frame_rate / self.config.sampling_rate
            personaplex_seq_length = (
                math.ceil(personaplex_seq_length * ratio)
                if personaplex_audio_codes is None
                else personaplex_seq_length
            )
            user_seq_length = math.ceil(user_seq_length * ratio) if user_audio_codes is None else user_seq_length

            if tokens_seq_length != personaplex_seq_length or tokens_seq_length != user_seq_length:
                raise ValueError(
                    "At least one of the 3 inputs of `PersonaplexForConditionalGeneration` doesn't have the same sequence length as the others."
                    "Make sure that they all have the same sequence length. Check the `PersonaplexForConditionalGeneration` docstrings for more information."
                )
